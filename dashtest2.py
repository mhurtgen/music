...
html.Div([
        dcc.Dropdown(id='dropdown',options=[{'label':'baroque','value':'1'}
                              ,{'label':'classical','value':'2'}  
                              ,{'label':'romantic','value':'3'},
                              {'label':'contemporary','value':'4'}],value='1',
...
def graph_update(dropdown_value):
    df = gc.getcomposerperiod(dropdown_value)
    x=df.iloc[:, 1]
    y=df.iloc[:, 0]
    fig = px.bar(df, x, y, orientation='h',height=400,title='Prolific composers')
